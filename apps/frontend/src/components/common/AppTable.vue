      ...
      <table class="min-w-full divide-y divide-slate-200">
        <thead class="bg-slate-50">
          <tr>
            ...
            <th v-if="hasActions" scope="col" class="px-4 py-3 text-right text-xs font-semibold text-slate-500 uppercase tracking-wider">
              Actions
            </th>
          </tr>
        </thead>
        <tbody class="divide-y divide-slate-100">
          ...
          <tr v-else>
            <td :colspan="columns.length + (hasActions ? 1 : 0)" class="px-4 py-12 text-center">
              <slot name="empty">
