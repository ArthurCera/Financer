<template>
  <AppLayout>
    ...
    <div class="flex flex-wrap items-center justify-between gap-4 mb-6">
      <div>
        <h1 class="text-xl font-bold text-slate-900">Income</h1>
        <p class="text-sm text-slate-500">Track all your income sources</p>
      </div>

      <div class="flex flex-wrap items-center gap-3">
        <select v-model="selectedMonth" class="input-base w-auto" @change="loadIncomes">
          <option v-for="m in months" :key="m.value" :value="m.value">{{ m.label }}</option>
        </select>
        <select v-model="selectedYear" class="input-base w-auto" @change="loadIncomes">
    ...
    <div class="mb-6 bg-emerald-50 border border-emerald-200 rounded-xl px-6 py-4 flex items-center justify-between">
      <div>
        <p class="text-sm font-medium text-emerald-700">Total Income this period</p>
        <p class="text-2xl font-bold text-emerald-700 mt-0.5">{{ formatCurrency(totalIncome) }}</p>
      </div>
    ...
    <!-- Error banner -->
    <div v-if="incomeStore.error" class="mb-4 rounded-xl bg-red-50 border border-red-200 px-4 py-3">
      <p class="text-sm text-red-700">{{ incomeStore.error }}</p>
    </div>
    ...
    <AppTable :columns="columns" :rows="tableRows">
      ...
      <template #empty>
        <div class="text-center py-12 text-slate-400">
          ...
          <p class="text-sm font-medium">No income records for this period</p>
          <p class="text-xs mt-1">Click "Add Income" to record your first income</p>
        </div>
      </template>
    </AppTable>

    <!-- Create/Edit Modal -->
    <AppModal v-model="showModal" :title="editingIncome ? 'Edit Income' : 'Add Income'">
      <IncomeForm
